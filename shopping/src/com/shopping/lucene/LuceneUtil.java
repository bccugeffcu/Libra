...
public class LuceneUtil {
	...
	public List<LuceneVo> searchIndex(String keyword, int start, int size,
		...
		try {
			...
			for (int i = start; i < end; i++) {
				...
				vo.setVo_add_time(CommUtil.null2Long(doc.get("add_time"))
						.longValue());
	...
	public LuceneResult search(String params, int pageNo, double begin_price,
		...
		try {
			...
			if (intPageNo < 1)
				intPageNo = 1;
			List vo_list = searchIndex(params, (intPageNo - 1) * this.pageSize,
	...
	private Document builderDocument(LuceneVo luceneVo) {
		...
		Field title = new Field("title", Jsoup.clean(luceneVo.getVo_title(),
				Whitelist.none()), Field.Store.YES, Field.Index.ANALYZED);
		title.setBoost(10.0F);
		Field content = new Field("content", Jsoup.clean(
				luceneVo.getVo_content(), Whitelist.none()), Field.Store.YES,
				Field.Index.ANALYZED);
		Field type = new Field("type", luceneVo.getVo_type(), Field.Store.YES,
				Field.Index.ANALYZED);
		...
		Field goods_salenum = new Field("goods_salenum",
				CommUtil.null2String(Integer.valueOf(luceneVo
						.getVo_goods_salenum())), Field.Store.YES,
				Field.Index.ANALYZED);
		document.add(id);
		document.add(title);
	...
	public static void main(String[] args) {
		...
		Date d1 = new Date();
		LuceneResult list = lucence.search("专柜正品黑色时尚冬装男", 0, 0.0D, 500.0D,
				null, null);
		Date d2 = new Date();
