...
public class StatManageAction {
	...
	public void execute() throws Exception {
		...
		List<MobileVerifyCode> mvcs = this.mobileverifycodeService
				.query("select obj from MobileVerifyCode obj where obj.addTime<=:time",
						params, -1, -1);
		...
		for (Store store : stores) {
			...
			evas = this.evaluateService
					.query("select obj from Evaluate obj where obj.of.store.id=:store_id and obj.addTime>=:addTime",
							params, -1, -1);
		...
		for (StoreClass sc : scs) {
			...
			List<StorePoint> sp_list = this.storePointService
					.query("select obj from StorePoint obj where obj.store.sc.id=:sc_id",
							params, -1, -1);
			...
			sc.setService_evaluate(BigDecimal.valueOf(CommUtil.div(
					Double.valueOf(service_evaluate),
					Integer.valueOf(sp_list.size()))));
			sc.setShip_evaluate(BigDecimal.valueOf(CommUtil.div(
			...
			if (ret) {
				...
				if (of.getPayment().getMark().equals("balance")) {
					...
					if (this.configService.getSysConfig().getBalance_fenrun() == 1) {
						...
						log11.setPd_log_amount(BigDecimal
						...
						buyer1.setFreezeBlance(BigDecimal.valueOf(CommUtil
								.subtract(buyer1.getFreezeBlance(),
										of.getTotalPrice())));
		...
		List<DeliveryGoods> dgs = this.deliveryGoodsService
				.query("select obj from DeliveryGoods obj where obj.d_goods.goods_store.delivery_end_time<:delivery_end_time",
						params, -1, -1);
		...
		stores = this.storeService
				.query("select obj from Store obj where obj.combin_end_time<=:combin_end_time",
						params, -1, -1);
		// Goods goods;
		for (Store store : stores) {
		...
		for (Goods goods1 : goods_list) {
			...
			goods1.setDescription_evaluate(BigDecimal
					.valueOf(description_evaluate));
			this.goodsService.update(goods1);
		}
		...
		List<Store> store_list = this.storeService
				.query("select obj from Store obj where obj.weixin_status=:weixin_status",
						params, -1, -1);
	...
	private boolean send_email(OrderForm order, String mark) throws Exception {
		...
		if (template.isOpen()) {
			...
			String path = System.getProperty("shopping.root") + "vm"
					+ File.separator;
			...
			p.setProperty("file.resource.loader.path",
					System.getProperty("shopping.root") + "vm" + File.separator);
