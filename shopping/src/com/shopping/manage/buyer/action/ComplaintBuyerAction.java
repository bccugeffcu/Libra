...
public class ComplaintBuyerAction {
	...
	public ModelAndView complaint(HttpServletRequest request,
		...
		if (!CommUtil.null2String(status).equals("")) {
			qo.addQuery(
					"obj.status",
					new SysMap("status", Integer.valueOf(CommUtil
							.null2Int(status))), "=");
		}
	...
	public ModelAndView complaint_save(HttpServletRequest request,
		...
		try {
			...
			if (((Map) map).get("fileName") != "") {
				...
				from_acc2.setName(CommUtil.null2String(((Map) map)
						.get("fileName")));
				from_acc2.setExt(CommUtil.null2String(((Map) map).get("mime")));
				from_acc2.setSize(CommUtil.null2Float(((Map) map)
						.get("fileSize")));
			...
			if (((Map) map).get("fileName") != "") {
				...
				from_acc3.setName(CommUtil.null2String(((Map) map)
						.get("fileName")));
				from_acc3.setExt(CommUtil.null2String(((Map) map).get("mime")));
				from_acc3.setSize(CommUtil.null2Float(((Map) map)
						.get("fileSize")));
