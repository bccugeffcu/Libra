...
public class AccountBuyerAction {
	...
	public ModelAndView account_password(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
			...
			mv = new JModelAndView("wap/account_password.html",
					this.configService.getSysConfig(),
	...
	public ModelAndView friend_search(HttpServletRequest request,
		...
		if ((years != null) && (!years.equals(""))) {
			...
			if (years.equals("18")) {
				qo.addQuery(
						"obj.years",
						new SysMap("years", Integer.valueOf(CommUtil
								.null2Int(years))), "<=");
			}
			if (years.equals("50")) {
				qo.addQuery(
			...
			if ((!years.equals("18")) && (!years.equals("50"))) {
				...
				qo.addQuery(
						"obj.years",
						new SysMap("years2", Integer.valueOf(CommUtil
								.null2Int(y[1]))), "<=");
			}
		}
		if ((sex != null) && (!sex.equals(""))) {
			...
			qo.addQuery("obj.sex",
					new SysMap("sex", Integer.valueOf(CommUtil.null2Int(sex))),
					"=");
		}
		if ((area_id != null) && (!area_id.equals(""))) {
			Area area = this.areaService
			...
			qo.addQuery("obj.area.id",
					new SysMap("area_id", CommUtil.null2Long(area_id)), "=");
		}
	...
	public void friend_add_save(HttpServletRequest request,
		...
		List sfs = this.sndFriendService
				.query("select obj from SnsFriend obj where obj.fromUser.id=:uid and obj.toUser.id=:user_id",
						params, -1, -1);
		if (sfs.size() == 0) {
