...
public class BaseBuyerAction {
	...
	public ModelAndView index(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
			...
			mv = new JModelAndView("wap/buyer_index.html",
					this.configService.getSysConfig(),
		...
		if (type.equals("2")) {
			...
			List myFriends = this.snsFriendService
					.query("select obj from SnsFriend obj where obj.fromUser.id=:f_uid",
							map, -1, -1);
		...
		if (type.equals("3")) {
			...
			List SnsAttentions = this.SnsAttentionService
					.query("select obj from SnsAttention obj where obj.fromUser.id=:uid ",
							params, -1, -1);
	...
	public ModelAndView dynamic_save(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/dynamic_list.html",
				this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		if ((goods_id != null) && (!goods_id.equals(""))) {
			...
			List hgcs = this.HomeGoodsClassService
					.query("select obj from HomePageGoodsClass obj where obj.user.id=:uid and obj.gc.id=:gc_id",
							params, -1, -1);
			if (hgcs.size() == 0) {
				Map map = new HashMap();
	...
	public ModelAndView dynamic_ajax_reply(HttpServletRequest request,
		...
		if ((parent_id != null) && (!parent_id.equals(""))) {
			parent = this.dynamicService.getObjById(Long.valueOf(Long
					.parseLong(parent_id)));
