...
public class StoreClassManageAction {
	...
	public ModelAndView add(HttpServletRequest request,
		...
		List scs = this.storeclassService
				.query("select obj from StoreClass obj where obj.parent is null order by obj.sequence asc",
						null, -1, -1);
		mv.addObject("scs", scs);
		if ((pid != null) && (!pid.equals(""))) {
			StoreClass obj = new StoreClass();
			obj.setParent(this.storeclassService.getObjById(Long.valueOf(Long
					.parseLong(pid))));
	...
	public String delete(HttpServletRequest request, String mulitId,
		...
		for (String id : ids) {
			if (!id.equals("")) {
				Set list = genericIds(this.storeclassService.getObjById(Long
						.valueOf(Long.parseLong(id))));
				Map params = new HashMap();
				params.put("ids", list);
				List<StoreClass> scs = this.storeclassService
						.query("select obj from StoreClass obj where obj.id in (:ids) order by obj.level desc",
								params, -1, -1);
	...
	public void ajax(HttpServletRequest request, HttpServletResponse response,
		...
		StoreClass obj = this.storeclassService.getObjById(Long.valueOf(Long
				.parseLong(id)));
	...
	public void storeclass_verify(HttpServletRequest request,
		...
		List gcs = this.storeclassService
				.query("select obj from StoreClass obj where obj.className=:className and obj.id!=:id",
						params, -1, -1);
