...
public class WxCommonUtil {
	...
	public static WxToken getToken(String appid, String appsecret) {
		...
		String requestUrl = WxToken_url.replace("APPID", appid).replace(
				"APPSECRET", appsecret);
		// 发起GET请求获取凭证
		...
		if (null != jsonObject) {
			try {
				...
				WxToken = null;
				// 获取WxToken失败
				log.error("获取WxToken失败 errcode:{} errmsg:{}",
	 ...
	 * 
	 * if(accessTocken!=null){ SimpleDateFormat sdf = new
	 * SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); java.util.Date end = new
	 * java.util.Date(); java.util.Date start = new
	 * java.util.Date(accessTocken.getAddTime().getTime());
	 * if(end.getTime()-accessTocken
	 * .getAddTime().getTime()>accessTocken.getExpires_in()*1000){ AccessToken
	 * accessToken = null; String requestUrl = access_token_url.replace("APPID",
	 * appid).replace("APPSECRET", appsecret); JSONObject jsonObject =
	 * httpRequest(requestUrl, "GET", null); // 如果请求成功 if (null != jsonObject) {
	 * try { accessToken = new AccessToken();
	 ...
	 * wrongMessage =
	 * "获取token失败 errcode:{} errmsg:{}"+jsonObject.getInt("errcode"
	 * )+jsonObject.getString("errmsg"); log.info(wrongMessage); } } return
	 * accessToken; } }
	 */
	 ...
	 * +accessTocken.getExpires_in
	 * ()+",addtime=now() where id='"+accessTocken.getId()+"'";
	 ...
	 * @return
	 */
	public static String createSignMD5(String characterEncoding,
		...
		String sign = Md5Encrypt.MD5Encode(sb.toString(), characterEncoding)
				.toUpperCase();
		return sign;
	}
	 ...
	 * @return
	 */
	public static String createSignSHA1(String characterEncoding,
