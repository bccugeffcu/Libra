...
public class AlipaySubmit {
	 ...
	 * @return 提交表单HTML文本
	 */
	public static String buildRequestWap(AlipayConfig config,
	 ...
	 * @return 要请求的参数数组
	 */
	private static Map<String, String> buildRequestParaWap(AlipayConfig config,
	...
	private static Map<String, String> buildRequestPara(AlipayConfig config,
		...
		sPara.put("sign", mysign);
		if (!((String) sPara.get("service"))
				.equals("alipay.wap.trade.create.direct")) {
			if (!((String) sPara.get("service"))
					.equals("alipay.wap.auth.authAndExecute"))
				sPara.put("sign_type", config.getSign_type());
		}
		return sPara;
	}
	 ...
	 * @return 签名结果字符串
	 */
	public static String buildRequestMysign(AlipayConfig config,
			Map<String, String> sPara) {
		String prestr = AlipayCore.createLinkString(sPara); // 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		String mysign = "";
		if (config.getSign_type().equals("RSA")) {
			mysign = RSA.signWap(prestr, AlipayConfig.private_key,
					config.getInput_charset());
		}
		return mysign;
	}
	...
	public static String query_timestamp(AlipayConfig config) throws Exception {
		...
		String strUrl = ALIPAY_GATEWAY_NEW + "service=query_timestamp&partner="
				+ config.getPartner() + "&_input_charset"
				+ config.getInput_charset();
