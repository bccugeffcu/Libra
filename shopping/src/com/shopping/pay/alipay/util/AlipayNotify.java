...
public class AlipayNotify {
	 ...
	 * @return 验证结果
	 */
	public static boolean verifyWap(AlipayConfig config,
	...
	public static Map<String, String> decrypt(AlipayConfig config,
		...
		inputPara.put("notify_data", RSA.decrypt(
				(String) inputPara.get("notify_data"),
				AlipayConfig.private_key, config.getInput_charset()));
		return inputPara;
	}

	public static boolean verifyNotifyWap(AlipayConfig config,
		...
		String responseTxt = "true";
		try {
			Document document = DocumentHelper.parseText((String) params
					.get("notify_data"));
	...
	private static boolean getSignVeryfy(AlipayConfig config,
		...
		if (config.getSign_type().equals("0001")) {
			isSign = RSA
					.verify(preSignStr, sign, AlipayConfig.getAli_public_key(),
							config.getInput_charset());
		}
		return isSign;
	}
	 ...
	 * @return 生成的签名结果
	 */
	private static boolean getSignVeryfyWap(AlipayConfig config,
			Map<String, String> Params, String sign) {
