...
public class CartViewAction {
	...
	private static Logger logger = LoggerFactory
			.getLogger(CartViewAction.class);
		...
		for (GoodsCart gc : list) {
			...
			if (CommUtil.null2String(gc.getCart_type()).equals("combin"))
				...
				total_price = CommUtil.null2Float(Double.valueOf(CommUtil.mul(
						Integer.valueOf(gc.getCount()), gc.getPrice())))
						+ total_price;
			}
		}
		...
		if (add) {
			...
			for (GoodsCart gc1 : sc33.getGcs()) {
				...
				if (CommUtil.null2String(gc1.getCart_type()).equals("")) {
					 ...
					 * (gc1.getGoods().getGoods_current_price()) *
					 * gc1.getCount();
					 */
					cart_total_price = cart_total_price
			...
			sc33.setTotal_price(BigDecimal.valueOf(CommUtil.formatMoney(Double
	...
	public void goods_count_adjust(HttpServletRequest request,
				...
				if (gg.getGg_count() >= CommUtil.null2Int(count))
					for (StoreCart sc1 : cart) { // sc = (StoreCart)gc.next();
						for (int i = 0; i < ((StoreCart) sc1).getGcs().size(); i++) {
							GoodsCart art = (GoodsCart) ((StoreCart) sc1)
									.getGcs().get(i);
							GoodsCart gc1 = art;
				...
				for (StoreCart scart : cart) {
					for (int i = 0; i < scart.getGcs().size(); i++) {
						...
						if (gcart.getId().toString().equals(cart_id)) {
							scart.setTotal_price(BigDecimal.valueOf(CommUtil
									.add(scart.getTotal_price(),
											Double.valueOf((CommUtil
													.null2Int(count) - gcart
													.getCount())
													* Double.parseDouble(gcart
															.getPrice()
															.toString())))));
				...
				for (StoreCart sscart : cart) {
					for (int i = 0; i < sscart.getGcs().size(); i++) {
						...
						if (((GoodsCart) gc).getId().toString().equals(cart_id)) {
							sscart.setTotal_price(BigDecimal.valueOf(CommUtil.add(
									sscart.getTotal_price(),
		...
		for (StoreCart ssscart : cart) {
			...
			if (ssscart.getStore().getId().equals(CommUtil.null2Long(store_id))) {
				((Map) map).put("sc_total_price", Float.valueOf(CommUtil
						.null2Float(ssscart.getTotal_price())));
			}
		}
	...
	public ModelAndView goods_cart1(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
			...
			mv = new JModelAndView("wap/goods_cart1.html",
					this.configService.getSysConfig(),
	...
	public ModelAndView goods_cart2(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/goods_cart2.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
	...
	public ModelAndView goods_cart3(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/goods_cart3.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
	...
	public ModelAndView order_pay_view(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/order_pay.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
		...
		if (of != null && of.getOrder_status() == 10) {
			...
			if (payments.size() > 0) {
				...
				if (payType.equals("weixin_wap")) {
					...
					sParaTemp.put("notify_url", siteURL
							+ "/alipay/alipay_notify.htm");
					sParaTemp.put("return_url", siteURL
							+ "/alipay/alipay_retrun.htm");
					sParaTemp.put("out_trade_no", of.getId().toString());
					sParaTemp.put("subject", "订单号为" + of.getOrder_id());
					// 价格测试改为1分钱
					// sParaTemp.put("total_fee", "0.01");
					sParaTemp.put("total_fee", of.getTotalPrice()
							.toPlainString());
					sParaTemp.put("show_url", "/index.htm");
					sParaTemp.put("body", "支付宝wap支付");
					// 其他业务参数根据在线开发文档，添加参数.文档地址:https://doc.open.alipay.com/doc2/detail.htm?spm=a219a.7629140.0.0.2Z6TSk&treeId=60&articleId=103693&docType=1

					String sHtmlText = AlipaySubmit.buildRequestWap(config,
							sParaTemp, "get", "确定");
					...
					return "redirect:" + CommUtil.getURL(request)
							+ "/index.htm?payMethodError";
				}

			} else {
				// 支付方式未配置
				return "redirect:" + CommUtil.getURL(request)
						+ "/index.htm?noPayMethod";
			}
	...
	public ModelAndView oauthCode(HttpServletRequest request,
			HttpServletResponse response) {
		logger.info("支付收到微信code回调请求");
		ModelAndView mv = new JModelAndView("wap/wxpay.html",
				this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		// 用户同意授权后，能获取到code
		...
		if (null != code && !"".equals(code) && !"authdeny".equals(code)) {
			...
			WxOauth2Token wxOauth2Token = WxAdvancedUtil.getOauth2AccessToken(
					of.getPayment().getWeixin_appId(), of.getPayment()
							.getWeixin_appSecret(), code);
			// 用户标识
			...
			mv.addObject("siteName", this.configService.getSysConfig()
					.getWebsiteName());
	...
	public void wxpay(HttpServletRequest request, HttpServletResponse response,
			String openId, String sn, String productName, String totalPrice,
		...
		if (of == null) {
			...
			/** 将元转换为分 */
			amount = of.getTotalPrice().multiply(new BigDecimal(100))
					.setScale(0).toString();
		...
		try {
			...
			String result = WxCommonUtil.httpsRequestString(UNI_URL, "POST",
					requestXML);
			 ...
			 * 注意参数名称的大小写
			 */
			String paySign = WxCommonUtil.createSignMD5("UTF-8", params,
					API_KEY);
			// 预支付单号
			params.put("packageValue", "prepay_id=" + map.get("prepay_id"));
			params.put("paySign", paySign); // 支付签名
			// 付款成功后同步请求的URL，请求我们自定义的支付成功的页面，展示给用户
			params.put("sendUrl", siteURL
					+ "/wechat/paysuccess.htm?totalPrice=" + totalPrice);
			...
			String ticktresult = WxCommonUtil.httpsRequestString(requestUrl,
					"GET", null);
			...
			String config_sign = WxCommonUtil
					.createSignSHA1("UTF-8", configMap);
	...
	public void wxcodepay(HttpServletRequest request,
		...
		if (of.getOrder_status() == 10) {
			...
			parameters.put("total_fee",
					of.getTotalPrice().multiply(new BigDecimal(100))
							.setScale(0).toString());// 总金额
			...
			// 支付成功后回调的action
			parameters.put("notify_url", CommUtil.getURL(request)
					+ "/wechat/paynotify.htm");// 支付成功后回调的action，与JSAPI相同
			...
			// API_KEY);
			String sign = WxCommonUtil.createSignMD5("UTF-8", parameters,
					payment.getWeixin_paySignKey());
			parameters.put("sign", sign);// 签名
			String requestXML = WxCommonUtil.getRequestXml(parameters);
			logger.info("requestXML" + requestXML);
			String result = WxCommonUtil.httpsRequestString(UNI_URL, "POST",
					requestXML);// WxCommonUtil.httpsRequest(WxConstants.UNIFIED_ORDER_URL,
								// "POST", requestXML);
			// System.out.println(" 微信支付二维码生成" + result);
			Map<String, String> map = new HashMap<String, String>();
			try {
				map = WxCommonUtil.doXMLParse(result);
				logger.info("------------------code_url=" + map.get("code_url")
						+ ";      result_code=" + map.get("code_url")
						+ "------------------------------");
			} catch (Exception e) {
			...
			if (returnCode.equalsIgnoreCase("SUCCESS")
				...
				String urls = request.getContextPath()
						...
						+ java.io.File.separator + "weixin_qr"
						+ java.io.File.separator + "wxpay"
	...
	public ModelAndView order_finish(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/order_finish.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
	...
	public ModelAndView cart_address(HttpServletRequest request,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/cart_address.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
	...
	public ModelAndView address(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("address.html",
				this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		String shopping_view_type = CommUtil.null2String(request.getSession()
				.getAttribute("shopping_view_type"));
		if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				&& (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/address.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
		}
		...
		CommUtil.saveIPageList2ModelAndView(url + "/address.htm", "", params,
				pList, mv);
		List areas = this.areaService.query(
				"select obj from Area obj where obj.parent.id is null", null,
				-1, -1);
