...
public class SpareGoodsViewAction {
	...
	public ModelAndView sparegoods_nav(HttpServletRequest request,
		...
		if ((SecurityUserHolder.getCurrentUser() != null)
			...
			List sps = this.sparegoodsService
					.query("select obj from SpareGoods obj where obj.status=:status and obj.down=:down and obj.user.id=:uid",
							params, -1, -1);
			...
			List drops = this.sparegoodsService
					.query("select obj from SpareGoods obj where obj.status=:status and obj.user.id=:uid",
							params, -1, -1);
			...
			List down = this.sparegoodsService
					.query("select obj from SpareGoods obj where obj.down=:down and obj.user.id=:uid",
							params, -1, -1);
		...
		List navs = this.navService
				.query("select obj from Navigation obj where obj.type=:type and obj.display=:display order by sequence asc",
						map, -1, -1);
	...
	public ModelAndView sparegoods(HttpServletRequest request,
			HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("sparegoods.html",
				this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		Map map = new HashMap();
		map.put("display", Boolean.valueOf(true));
		List floors = this.sparegoodsfloorService
				.query("select obj from SpareGoodsFloor obj where obj.display=:display order By sequence asc",
						map, -1, -1);
		List sgcs = this.sparegoodsclassService
				.query("select obj from SpareGoodsClass obj where obj.parent.id is null order by sequence asc",
						null, -1, -1);
	...
	public ModelAndView sparegoods_search(HttpServletRequest request,
		...
		if ((orderBy != null) && (!orderBy.equals(""))) {
			if (orderBy.equals("recommend")) {
				qo.addQuery("obj.recommend", new SysMap("obj_recommend",
			...
			if ((price_end != null) && (!price_end.equals(""))) {
				qo.addQuery(
						"obj.goods_price",
						new SysMap("goods_end", Integer.valueOf(CommUtil
								.null2Int(price_end))), "<=");
			}
		}
		if ((keyword != null) && (!keyword.equals(""))) {
			qo.addQuery("obj.title",
					new SysMap("obj_title", "%" + keyword.trim() + "%"), "like");
		}
